...
func (repository HiveRepository) GetAll(request *messaging.ObjectRequest) RepositoryResponse {
	...
	if isError == false {
		tableName := repository.getDatabaseName(request) + "." + request.Controls.Class
		...
		if er == nil && err == nil {
			...
			for {
				...
				if row == "" {
					...
					delete(myMap, request.Controls.Class+".osid")
...
func (repository HiveRepository) GetByKey(request *messaging.ObjectRequest) RepositoryResponse {
	...
	if isError == false {
		...
		fmt.Println("SELECT * FROM " + tableName + " where osid='" + key + "'")
		er, err := conn.Client.Execute("SELECT * FROM " + tableName + " where osid='" + key + "'")
		if er == nil && err == nil {
...
func (repository HiveRepository) InsertMultiple(request *messaging.ObjectRequest) RepositoryResponse {
	...
	if isError == true {
		...
		isDBReady := checkDBAndCreateOnNotAvailbale(repository.getDatabaseName(request), conn)
...
func (repository HiveRepository) verifyTable(conn *hive.HiveConnection, obj map[string]interface{}) {
	query := "create database if not exists " + database + ";"
	repository.executeNonQuery(conn, query)
}

func (repository HiveRepository) executeNonQuery(conn *hive.HiveConnection, query string) (status bool) {
	fmt.Print(query)
	er, err := conn.Client.Execute(query)
	if er != nil && err != nil {
		status = false
	} else {
		status = true
