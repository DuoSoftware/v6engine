...
type HTTPService2 struct {
	...
	getByKey           gorest.EndPoint `method:"GET" path:"/ObjectStore/{namespace:string}/{class:string}/{id:string}/" output:"string"`
	//getResult          gorest.EndPoint `method:"GET" path:"/aa/{namespace:string}/{class:string}" output:"string"`
	// postData   gorest.EndPoint `method:"POST" path:"/{namespace:string}/{class:string}" output:"string"`
	// putData    gorest.EndPoint `method:"PUT" path:"/{namespace:string}/{class:string}" output:"string"`
	// deleteData gorest.EndPoint `method:"DELETE" path:"/{namespace:string}/{class:string}" output:"string"`
}

func (h HTTPService2) GetByKey(namespace, class, id string) (output string) { // res and req are injected by Martini
	...
	responseMessage, isSuccess := dispatchRequest(h.Context.Request(), params)

	if isSuccess {
		h.ResponseBuilder().SetResponseCode(200).WriteAndOveride([]byte(responseMessage))
	} else {
		h.ResponseBuilder().SetResponseCode(500).WriteAndOveride([]byte("fekd up"))
	}

	return
}

func (h HTTPService2) Verify() (output string) {
	fmt.Println("huehuehue")
	output = "{\"name\": \"Go Rest Check\",\"version\": \"6.0.13-a\",\"Change Log\":\"Added doc cache!\",\"author\": {\"name\": \"Duo Software\",\"url\": \"http://www.duosoftware.com/\"},\"repository\": {\"type\": \"git\",\"url\": \"https://github.com/DuoSoftware/v6engine/\"}}"
	return
}
...
func (h *HTTPService2) Start(isLogEnabled bool, isJsonStackEnabled bool) {
...
func versionHandler(params martini.Params, w http.ResponseWriter, r *http.Request) {
	versionData := "{\"name\": \"Objectstore\",\"version\": \"1.2.5-a\",\"Change Log\":\"test branch!\",\"author\": {\"name\": \"Duo Software\",\"url\": \"http://www.duosoftware.com/\"},\"repository\": {\"type\": \"git\",\"url\": \"https://github.com/DuoSoftware/v6engine/\"}}"
