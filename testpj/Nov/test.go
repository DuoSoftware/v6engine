...
func main() {
	...
	//fmt.Println(queryparser.GetElasticQuery("SELECT tttt, Id, age from Student s1, game g2 where table = 5@00 AND kappa not between 50 AND 100 AND kat not in ('asdf', 'wert') OR Country LIKE '%land%' order by ppds, ggg desc;", "com.duoworld.com", "test"))
